   ...
   useEffect(() => {
	...
	function getDefaultOption() {
		if (!props.country) {
			...
			// return <option value="" className="dark:text-blacklight">Select state</option>;
		}
		return null;
	}
  return (
	<div className="relative">
	  	<select className={`${props.className} w-full cursor-pointer appearance-none dark:bg-transparent dark:text-blacklight focus:outline-none pr-4`}  onChange={(e) => props.onChange(e.target.value)}>
			{getDefaultOption()}
			{getRegionList()}
		</select>
